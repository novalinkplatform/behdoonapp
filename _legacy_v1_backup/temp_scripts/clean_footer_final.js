...
const islandStartStr = '<!-- ================= ISLAND FOOTER ================= -->';
const islandEndStr = '<!-- Mobile Sticky CTA -->';

const islandStart = content.indexOf(islandStartStr);
const islandEnd = content.indexOf(islandEndStr);

if (islandStart !== -1 && islandEnd !== -1) {
    content = content.substring(0, islandStart) + newFooterHTML + '\n    ' + content.substring(islandEnd);
    console.log('Island footer completely removed and replaced.');
} else {
    console.log('Could not find island footer bounds.');
}
