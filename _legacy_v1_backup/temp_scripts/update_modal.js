...
const oldModalStart = frontend.indexOf('<!-- Submit Request Modal -->');
const oldModalEndStr = '</div>\n</div>\n\n<script>';
const oldModalEnd = frontend.indexOf(oldModalEndStr, oldModalStart);

if(oldModalStart !== -1 && oldModalEnd !== -1) {
    // We should also remove the old scripts: openModal, closeModal, submitRequest.
    const oldScriptStart = frontend.indexOf('<script>\nfunction openModal(serviceName = null)');
    const oldScriptEnd = frontend.indexOf('</script>\n\n</body>', oldScriptStart);
    
    if (oldScriptStart !== -1 && oldScriptEnd !== -1) {
        // Remove script
        frontend = frontend.substring(0, oldScriptStart) + frontend.substring(oldScriptEnd + 10);
    }
    
    // Remove modal HTML
    // the old script was right after it actually! Wait, the structure was:
    // <!-- Submit Request Modal --> ... </div></div>
    // <script> function openModal ... </script>
    
    // Let's just find "<!-- Submit Request Modal -->" and remove everything up to "</body>"
    const bodyEnd = frontend.indexOf('</body>', oldModalStart);
    if(bodyEnd !== -1) {
        frontend = frontend.substring(0, oldModalStart) + frontend.substring(bodyEnd);
    }
}

// Now we need to append the NEW modal to `html` (before `</body>`).
// Actually, `add_modal.js` already added the new modal to `footerHTML`.
// Does the homepage (`html`) use `footerHTML`? NO. It has its own footer!
// Let's add the new modalHTML to the end of `html`.
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden transform scale-95 transition-transform duration-300 flex flex-col max-h-[90vh]" id="requestModalContent">
        ...
        <div class="p-6 overflow-y-auto flex-1 hide-scrollbar relative">
            ...
            <div id="step3" class="modal-step hidden">
                <h4 class="font-bold text-slate-700 mb-4">توضیحات خرابی یا درخواست خود را بنویسید (اختیاری)</h4>
            ...
            <div id="stepSuccess" class="modal-step hidden text-center py-6">
                ...
                <button type="button" onclick="closeRequestModal()" class="mt-6 text-[#8B1C31] font-bold text-sm">بستن پنجره</button>
            </div>
...
// Insert into `html`
const bodyEndIndex = frontend.indexOf('</body>', frontend.indexOf('export const html = `'));
if(bodyEndIndex !== -1) {
    frontend = frontend.substring(0, bodyEndIndex) + modalHTML + frontend.substring(bodyEndIndex);
}
