...
if (startIdx !== -1 && endIdx !== -1) {
    content = content.substring(0, startIdx) + 'const servicesData = ' + newServicesData + content.substring(endIdx + 1);
    
    // Now replace the homepage cards
    const newHomepageCards = `
    <!-- MAIN SERVICES GRID -->
    ...
    const gridStart = content.indexOf('<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">');
    // Find the end of this div by looking for the section close or next section
    // Fortunately, right after this grid is `</div></div></section>` or `<!-- Features -->`
    const featuresStart = content.indexOf('<!-- Features -->');
    if (gridStart !== -1 && featuresStart !== -1) {
        // extract the part before grid
        const beforeGrid = content.substring(0, gridStart);
        // We know there are two `</div>` and a `</section>` between the grid and `<!-- Features -->`.
        // Let's just find the `</div>` that corresponds to the container.
        const afterGrid = content.substring(featuresStart - 40); // roughly skip backwards to keep </section>
        
        // Let's do a safer regex replacement for the grid:
        const gridRegex = /<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">[\s\S]*?<\/a>\s*<\/div>/;
        content = content.replace(gridRegex, newHomepageCards);
    }
    
    // Replace the quick links in the hero section too
    content = content.replace('href="/services/tasisat"', 'href="/services/hvac"');
    content = content.replace('>تأسیسات<', '>سرمایش و گرمایش<');
    
    content = content.replace('href="/services/electrical"', 'href="/services/electrical"');
    // electrical is same ID, just rename tag
    content = content.replace('>برق‌کاری<', '>برقکاری<');
    
    content = content.replace('href="/services/renovation"', 'href="/services/renovation"');
    content = content.replace('>بازسازی<', '>تعمیرات و بازسازی ساختمان<');

    // Remove the 4th quick tag if we want, or map it to plumbing
    // The existing 4th was "بنایی" -> /services/construction
    content = content.replace('href="/services/construction"', 'href="/services/plumbing"');
    content = content.replace('>بنایی<', '>لوله کشی<');

    fs.writeFileSync('worker.js', content);
    console.log('Categories restructured successfully!');
} else {
    console.log('Could not find servicesData bounds');
}
