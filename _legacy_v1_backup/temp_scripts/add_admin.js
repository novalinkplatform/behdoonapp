        ...
        <div class="flex-1 md:mr-64 p-6 md:p-10">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="p-6 border-b border-slate-100 flex justify-between items-center">
                    ...
                    <a href="#" class="text-sm font-bold text-brand-600 hover:text-brand-700">مشاهده همه</a>
                </div>
                    ...
                    <table class="w-full text-right border-collapse">
                        ...
                        <tbody class="text-sm">
                            <tr class="hover:bg-slate-50 border-b border-slate-50 transition-colors">
                                ...
                                <td class="p-4"><button class="text-brand-600 font-bold hover:underline">مشاهده</button></td>
                            </tr>
                            <tr class="hover:bg-slate-50 border-b border-slate-50 transition-colors">
...
// Insert adminHTML definition near magazineHTML
const insertPos = currentWorker.indexOf('let htmlResponse = \'\';');

currentWorker = currentWorker.substring(0, insertPos) + '\n    const adminHTML = `' + adminHTML.replace(/\`/g, '\\`') + '`;\n\n    ' + currentWorker.substring(insertPos);

// Modify routing
const routingTarget = `let htmlResponse = '';
    if (path === '/magazine' || path === '/magazine/') {`;
const routingReplacement = `let htmlResponse = '';
...
if (currentWorker.includes(routingTarget)) {
    currentWorker = currentWorker.replace(routingTarget, routingReplacement);
