...
// Helper to replace the old footer block
function replaceOldFooter(content) {
    const startTag = '<footer class="bg-slate-900';
    let startIndex = content.indexOf(startTag);
    let replacedCount = 0;
    while(startIndex !== -1) {
        // Find closing tag </footer> after startIndex
        const endTag = '</footer>';
        const endIndex = content.indexOf(endTag, startIndex);
        
        if (endIndex !== -1) {
            content = content.substring(0, startIndex) + newFooterHTML + content.substring(endIndex + endTag.length);
            replacedCount++;
            startIndex = content.indexOf(startTag, startIndex + newFooterHTML.length);
        } else {
            break;
        }
    }
    console.log('Replaced ' + replacedCount + ' footers.');
    return content;
}

content = replaceOldFooter(content);

// Ensure there's no leftover `<!-- ================= 3-ROW FOOTER ================= -->` tags if we replaced the `<footer class="bg-slate-900`
// Actually `newFooterHTML` has `<!-- ================= ISLAND FOOTER v2 ================= -->`
// The old tags `<!-- ================= 3-ROW FOOTER ================= -->` might still exist right before `<footer class="bg-slate-900...` but we didn't replace them, so there might be duplicate comments. We can clean that up.
content = content.replace(/<!-- ================= 3-ROW FOOTER ================= -->/g, '');
