...
const newFooterHTML = `
    <!-- ================= ISLAND FOOTER v2 ================= -->
    <footer class="bg-slate-50 pt-16 pb-28 md:pb-12 border-t border-slate-200">
...
// 1. Replace the footer in `const html`
const footerStartString = '<!-- ================= 3-ROW FOOTER ================= -->';
const homeFooterStart = content.indexOf(footerStartString);
if (homeFooterStart !== -1) {
    const homeFooterEnd = content.indexOf('<!-- Mobile Sticky CTA -->', homeFooterStart);
    if (homeFooterEnd !== -1) {
        content = content.substring(0, homeFooterStart) + newFooterHTML + '\n    ' + content.substring(homeFooterEnd);
        console.log('Replaced footer in home HTML.');
    } else {
        console.log('Could not find end of footer in home HTML.');
    }
} else {
    console.log('Could not find 3-ROW FOOTER tag in home HTML.');
}

// 2. Replace the footer in `footerHTML`
const footerHTMLLiteralStart = content.indexOf('const footerHTML = `');
if (footerHTMLLiteralStart !== -1) {
    // The literal starts with 'const footerHTML = `\n    <footer class=\"bg-slate-900...'
    // Wait, the previous footerHTML actually had a `<footer class="bg-slate-900...` inside it.
    // Let's just find the backtick ends.
    const backtickEnd = content.indexOf('`;', footerHTMLLiteralStart);
    if (backtickEnd !== -1) {
        content = content.substring(0, footerHTMLLiteralStart) + 'const footerHTML = `\n' + newFooterHTML + '\n' + content.substring(backtickEnd);
        console.log('Replaced footer in footerHTML.');
    }
}
