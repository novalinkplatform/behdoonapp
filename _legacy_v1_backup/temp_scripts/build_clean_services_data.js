...
for (const cat of ['hvac', 'plumbing', 'electrical', 'renovation']) {
    const data = JSON.parse(fs.readFileSync(`${cat}_data.json`, 'utf8'));
    const mappings = slugsMap[cat] || [];
    
    // Inject slugs into subservices
    if (data.subServices) {
        data.subServices.forEach((sub, idx) => {
            const m = mappings[idx];
            if (m) {
                sub.slug = m.slug;
                sub.persianSlug = m.persianSlug;
            }
        });
    }
    
    fullServicesData[cat] = data;
}

console.log('fullServicesData assembled successfully!');
for (const [k, v] of Object.entries(fullServicesData)) {
    console.log(`- ${k}: title="${v.title.slice(0, 25)}...", subs=${v.subServices.length}`);
    v.subServices.forEach((s, i) => {
        if (!s.slug) console.error(`Missing slug in ${k}[${i}]!`);
    });
}

fs.writeFileSync('all_services_data.json', JSON.stringify(fullServicesData, null, 2), 'utf8');
console.log('Saved to all_services_data.json, size:', fs.statSync('all_services_data.json').size);
