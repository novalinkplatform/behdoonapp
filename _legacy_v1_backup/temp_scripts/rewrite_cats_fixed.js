...
const endIdx = content.indexOf('function renderServicePage');

if (startIdx !== -1 && endIdx !== -1) {
    // Keep everything up to the start, insert our new data, and add a blank line before renderServicePage
    content = content.substring(0, startIdx) + 'const servicesData = ' + newServicesData + '\n\n' + content.substring(endIdx);
    ...
    content = content.replace(gridRegex, newCards);

    // Update Quick Tags
    content = content.replace(/href="\/services\/tasisat"/g, 'href="/services/hvac"');
    content = content.replace(/>تأسیسات</g, '>سرمایش و گرمایش<');
    content = content.replace(/>برق‌کاری</g, '>برقکاری<');
    content = content.replace(/>بازسازی</g, '>تعمیرات و بازسازی ساختمان<');
    content = content.replace(/href="\/services\/construction"/g, 'href="/services/plumbing"');
    content = content.replace(/>بنایی</g, '>لوله کشی<');

    fs.writeFileSync('worker.js', content);
    console.log('Categories restructured completely!');
} else {
    console.log('Bounds still not found!', startIdx, endIdx);
}
