...
// The worker has `homeHTML` stored in `const html = `...``
// Inside `const html`, there's the `<main>` tag. Everything after `</main>` until `<!-- Fixed Mobile CTA -->` needs to be replaced.
// First, check if there's an `ISLAND FOOTER` comment and replace until `<!-- Fixed Mobile CTA -->`.
const islandFooterStr = '<!-- ================= ISLAND FOOTER ================= -->';
const fixedCTAStr = '<!-- Fixed Mobile CTA -->';
const footerReplacementPoint = content.indexOf(islandFooterStr);
const endOfFooterPoint = content.indexOf(fixedCTAStr);

if (footerReplacementPoint !== -1 && endOfFooterPoint !== -1) {
    // There are multiple places this might happen.
    // Replace all text between `islandFooterStr` and `fixedCTAStr` with `newFooterHTML`.
    // Wait, let's just do it cleanly.
}

// Since there is a headerHTML and footerHTML variables from my router script, I must replace `const footerHTML = \`...\`` 
// with the new footer HTML.
const routerFooterStart = content.indexOf('const footerHTML = `');
if (routerFooterStart !== -1) {
    const nextTick = content.indexOf('`;', routerFooterStart);
    if (nextTick !== -1) {
        content = content.substring(0, routerFooterStart) + 'const footerHTML = `\n' + newFooterHTML + '\n' + content.substring(nextTick);
    }
}

// Now replace in `const html = `...`` (the home page html)
const homeIslandStart = content.indexOf('<!-- ================= ISLAND FOOTER ================= -->');
if (homeIslandStart !== -1) {
    const homeFixedCTA = content.indexOf('<!-- Fixed Mobile CTA -->', homeIslandStart);
    if (homeFixedCTA !== -1) {
        content = content.substring(0, homeIslandStart) + newFooterHTML + '\n    ' + content.substring(homeFixedCTA);
    }
}

// Ensure there is no second footer lingering in the home page
const doubleFooterRegex = /<footer class="bg-slate-900[\s\S]*?<\/footer>[\s\S]*?<footer class="bg-slate-900[\s\S]*?<\/footer>/;
if (doubleFooterRegex.test(content)) {
    // Remove the first occurrence if it happens to be duplicated
    content = content.replace(/<footer class="bg-slate-900[\s\S]*?<\/footer>\s*/, '');
}

fs.writeFileSync('worker.js', content);
console.log('Cleaned up footers and applied the 3-row slate footer.');
