// کادر ورود کد پیامکی — هر رقم یک خانه‌ی جدا (الگوی استاندارد اپ‌های بانکی/۲مرحله‌ای)، با پرشدن خودکار
// فوکوس، پیست کل کد در یک‌ضربه، شمارش‌معکوس ۶۰ثانیه‌ای برای «ارسال دوباره»، و — روی گوشی‌های اندروید با
// کروم — پرشدن خودکار کد از پیامک با WebOTP API (بدون هیچ تایپی). سه جای مختلف همین یک کامپوننت را
// صدا می‌زنند (ورود دومرحله‌ای، فعال‌سازی پیامکی، و مشابهش در سایت مشتری) تا تجربه‌ی یکسان و درست باشد.
...
export function renderOtpInputMarkup(idPrefix: string, length = 6): string {
  ...
  return `
    ...
    <div class="otp-resend-row">
      ...
      <button type="button" class="otp-resend-btn" id="${idPrefix}-resend-btn" hidden>ارسال دوباره‌ی کد</button>
    </div>
  `;
}
...
export function initOtpInput(
  ...
  function startTimer(): void {
    ...
    const tick = () => {
      ...
      timerEl!.textContent = `ارسال دوباره تا ${mm}:${ss}`;
      if (remaining <= 0) {
        if (intervalId) window.clearInterval(intervalId);
        timerEl!.hidden = true;
